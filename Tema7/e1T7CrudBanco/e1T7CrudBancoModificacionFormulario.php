...
// Conexión a la base de datos
      ...
      $consulta = $conexion->query("SELECT dni FROM cliente WHERE dni=".$_POST['dniModificacion']);
      $dniModificacion = $_POST['dniModificacion'];
      if ($consulta->rowCount() != 1) {
      ?>
        No existe un cliente con DNI <?= $_POST['dniModificacion'] ?><br>
        Por favor, vuelva a la <a href="e1T7CrudBanco.php">página de alta de cliente</a>.
      <?php
      } else { 
      ?>
      <h3>Nuevos datos de cliente con DNI <?= $_POST['dniModificacion'] ?></h3>
      <form action="e1T7CrudBancoModificacion.php" method="post">
        Introduzca los datos necesarios para modificar al cliente:<br>
        DNI <input type="text" name="dniModificado" required><br>
        Nombre <input type="text" name="nombreModificado"><br>
        Dirección <input type="text" name="direccionModificado"><br>
        Teléfono <input type="tel" name="telefonoModificado"><br>
        <input type="hidden" name="dniModificacion" value="<?= $_POST['dniModificacion'] ?>">
