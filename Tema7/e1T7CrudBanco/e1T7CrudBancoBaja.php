...
// Conexión a la base de datos
      ...
      $consulta = $conexion->query("SELECT dni FROM cliente WHERE dni=".$_POST['dniBaja']);
      
      if ($consulta->rowCount() != 1) {
      ?>
        No existe un cliente con DNI <?= $_POST['dniBaja'] ?><br>
        Por favor, vuelva a la <a href="e1T7CrudBanco.php">página de alta de cliente</a>.
      <?php
      } else { //DELETE FROM fruit WHERE colour = 'red'"
        $insercion = "DELETE FROM cliente WHERE dni = ('$_POST[dniBaja]')";
        //echo $insercion;
